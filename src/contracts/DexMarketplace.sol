...
interface IDexBundleMarketplace {
...
contract DexMarketplace is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    IDexAddressRegistry public addressRegistry;
    ...
    function _buyItem(
        ...
        IDexBundleMarketplace(addressRegistry.bundleMarketplace())
    ...
    function createOffer(
        ...
        IDexAuction auction = IDexAuction(addressRegistry.auction());
    ...
    function registerRoyalty(
        ...
        require(_isDexNFT(_nftAddress), "invalid nft address");
    ...
    function _isDexNFT(address _nftAddress) internal view returns (bool) {
        return
            addressRegistry.artion() == _nftAddress ||
            IDexNFTFactory(addressRegistry.factory()).exists(_nftAddress) ||
            IDexNFTFactory(addressRegistry.privateFactory()).exists(
                _nftAddress
            ) ||
            IDexNFTFactory(addressRegistry.artFactory()).exists(
                _nftAddress
            ) ||
            IDexNFTFactory(addressRegistry.privateArtFactory()).exists(
                _nftAddress
            );
    }
    ...
    function getPrice(address _payToken) public view returns (int256) {
        int256 unitPrice;
        uint8 decimals;
        IDexPriceFeed priceFeed = IDexPriceFeed(
    ...
    /**
     @notice Update DexAddressRegistry contract
     @dev Only admin
     */
    function updateAddressRegistry(address _registry) external onlyOwner {
        addressRegistry = IDexAddressRegistry(_registry);
    }
        ...
        require(
            _payToken == address(0) ||
                (addressRegistry.tokenRegistry() != address(0) &&
                    IDexTokenRegistry(addressRegistry.tokenRegistry())
                        .enabled(_payToken)),
            "invalid pay token"
        );
    }
