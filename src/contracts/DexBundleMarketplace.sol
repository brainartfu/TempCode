...
contract DexBundleMarketplace is
    OwnableUpgradeable,
    ReentrancyGuardUpgradeable
{
        ...
        require(
            _payToken == address(0) ||
                (tokenRegistry != address(0) &&
                    IDexTokenRegistry(tokenRegistry).enabled(_payToken)),
            "invalid pay token"
        );
    ...
    function _buyItem(string memory _bundleID, address _payToken) private {
        ...
        if (_payToken == address(0)) {
            ...
            require(
                feeTransferSuccess,
                "DexMarketplace: Fee transfer failed"
            );
            ...
            require(
                ...
                "DexMarketplace: Owner transfer failed"
            );
        } else {
        ...
        for (uint256 i; i < listing.nfts.length; i++) {
            ...
            IDexMarketplace(addressRegistry.marketplace()).validateItemSold(
                listing.nfts[i],
                listing.tokenIds[i],
                owner,
                _msgSender()
        ...
        emit ItemSold(
            ...
            _bundleID,
            _payToken,
            IDexMarketplace(addressRegistry.marketplace()).getPrice(_payToken),
            price
        );
        ...
        nonReentrant
    {
        ...
        emit ItemSold(
            ...
            IDexMarketplace(addressRegistry.marketplace()).getPrice(address(offer.payToken)),
            offer.price
        );
