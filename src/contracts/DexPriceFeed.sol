...
contract DexPriceFeed is Ownable {
    /// @notice keeps track of oracles for each tokens
    mapping(address => address) public oracles;

    /// @notice Dex address registry contract
    ...
    function registerOracle(address _token, address _oracle)
        ...
        onlyOwner
    {
        IDexTokenRegistry tokenRegistry = IDexTokenRegistry(
            IDexAddressRegistry(addressRegistry).tokenRegistry()
        );
